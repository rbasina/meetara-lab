...
class BaseModelDownloader:
    """Downloads and manages base models for A_universal_full with resume capability"""
    
    def __init__(self):
        self.base_dir = Path(__file__).parent.parent.parent
        self.base_models_dir = self.base_dir / "models" / "A_universal_full" / "base_models"
    ...
    def check_model_status(self, model_name: str) -> Tuple[bool, str]:
        ...
        model_filename = f"{model_name.replace('/', '_')}_Q2_K.gguf"
    ...
    def download_base_model(self, model_name: str, force_redownload: bool = False) -> bool:
        """Download and convert a single base model to Q2_K GGUF"""
    ...
    def _get_expected_model_size(self, model_name: str) -> int:
        """Get expected file size for each model in Q2_K quantization"""
        # Approximate sizes in bytes for Q2_K quantization
        size_map = {
            "HuggingFaceTB/SmolLM2-1.7B": 500 * 1024**2,  # 500MB
            "microsoft/Phi-3.5-mini-instruct": 800 * 1024**2,  # 800MB
            "Qwen/Qwen2.5-7B-Instruct": 2000 * 1024**2,  # 2GB
            "microsoft/Phi-3-medium-4k-instruct": 1500 * 1024**2,  # 1.5GB
            "Qwen/Qwen2.5-14B-Instruct": 3500 * 1024**2,  # 3.5GB
            "microsoft/Phi-3-medium-14B-instruct": 3000 * 1024**2,  # 3GB
        }
        return size_map.get(model_name, 1000 * 1024**2)  # Default 1GB
