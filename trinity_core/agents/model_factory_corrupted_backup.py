...
class IntelligentModelFactory:
    ...
    def __init__(self, intelligence_level: IntelligenceLevel = IntelligenceLevel.AUTONOMOUS):
        ...
        # Get llama.cpp path from the central config
        self.llama_cpp_path = self.config_manager.get_llama_cpp_path()
        if not self.llama_cpp_path:
            raise ValueError("LLaMA.cpp path not found in config. Please run scripts/setup/setup_llama_cpp.py")

        self.converter_path = self.llama_cpp_path / "convert-hf-to-gguf.py"
        quantize_exe_name = "quantize"
        self.quantizer_path = self.llama_cpp_path / quantize_exe_name
    ...
    def _check_dependencies(self):
        """Checks if all required executables are present."""
        if not self.converter_path.exists():
            logger.warning(f"LLaMA.cpp converter not found at {self.converter_path}. This may cause errors at runtime.")
        if not self.quantizer_path.exists():
            raise FileNotFoundError(f"LLaMA.cpp quantizer not found at {self.quantizer_path}")
        self.logger.info("✅ LLaMA.cpp dependencies are met.")
