...
class TrinityIntelligenceHub:
    ...
    async def generate_data_for_domain(self, domain: str, sample_count: int, quality_target: float, simulation: bool) -> Dict[str, Any]:
        """
...
class TrinityDataGenerator:
    ...
    async def generate_intelligent_data(self, request: Dict[str, Any], 
                                      intelligence: Dict[str, Any]) -> Dict[str, Any]:
        """Generate intelligent training data with Trinity optimization"""
        
        domain = request.get("domain", "unknown")
        model_tier_name = request.get("model_tier", "balanced") # Default to balanced
        
        # Get model tier configuration from the centralized config manager
        model_tier_config = self.hub.config_manager.get_model_tier_config(model_tier_name)
        if not model_tier_config:
            logger.error(f"Could not find configuration for model_tier: {model_tier_name}. Defaulting sample count.")
            target_sample_count = 1000 # Fallback default
        else:
            target_sample_count = model_tier_config.get("sample_count", 1000)

        cache_key = self._generate_cache_key(request, intelligence)
        if self.hub and self.hub.intelligent_caching and cache_key in self.hub.shared_context["data_patterns"]:
            ...
            return {
                ...
                "domain": domain,
                "model_tier": model_tier_name,
                "target_sample_count": target_sample_count,
                "generated_sample_count": len(cached_pattern.get("training_examples", [])),
                "training_examples": cached_pattern.get("training_examples", []),
        ...
        generated_data = {
            "domain": domain,
            "training_examples": await self._generate_training_examples(domain, user_needs, target_sample_count),
            "quality_metrics": await self._assess_quality_metrics(domain),
            "optimizations": await self._suggest_optimizations(intelligence)
        }
        ...
        result = {
            ...
            "model_tier": model_tier_name,
            "target_sample_count": target_sample_count,
            "generated_sample_count": len(generated_data.get("training_examples", [])),
            "training_examples": generated_data.get("training_examples", []),
        ...
        if result.get("patterns"):
             ...
             self.hub.shared_context["data_patterns"][cache_key] = result # Simplified caching
        
        return result
    
    async def _generate_training_examples(self, domain: str, user_needs: Dict[str, Any], target_sample_count: int) -> List[Dict[str, Any]]:
        """Generate training examples and save them to a file."""
        
        # Define output directory and ensure it exists
        output_dir = Path("data/synthesis") / domain
        ...
        for i in range(target_sample_count):
            examples.append({
                "prompt": f"This is prompt number {i+1} for the {domain} domain.",
                "response": f"This is a sample response for the {domain} domain, demonstrating a {user_needs.get('primary_need', 'general')} need."
            })
        
        cleaned_examples = self._clean_generated_data(examples)

        # Save to file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = output_dir / f"simulated_data_{domain}_{timestamp}.json"
        
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(cleaned_examples, f, indent=2)
            logger.info(f"✅ Successfully generated and saved {len(cleaned_examples)} samples to {output_file}")
        except Exception as e:
            logger.error(f"❌ Failed to save simulated data for {domain}: {e}")
            # Even if saving fails, return the data for in-memory use
            
        return cleaned_examples

    def _clean_generated_data(self, examples: List[Dict[str, str]]) -> List[Dict[str, str]]:
        """Performs basic cleaning on the generated data."""
        seen = set()
        cleaned = []
        for item in examples:
            # Normalize whitespace and create a unique key for deduplication
            prompt = " ".join(item.get("prompt", "").split())
            response = " ".join(item.get("response", "").split())
            
            # Use a tuple of the cleaned prompt and response to identify duplicates
            item_key = (prompt, response)

            if prompt and response and item_key not in seen:
                seen.add(item_key)
                cleaned.append({"prompt": prompt, "response": response})
        
        if len(examples) > len(cleaned):
            logger.info(f"Data Cleaning: Removed {len(examples) - len(cleaned)} duplicate or empty entries.")

        return cleaned

    async def _assess_quality_metrics(self, domain: str) -> Dict[str, Any]:
        """Assess quality metrics of generated data"""
        # Placeholder for more complex quality assessment
        return {
...
class TrinityKnowledgeTransfer:
    ...
    def _load_domain_relationships_from_config(self) -> Dict[str, List[str]]:
        ...
        try:
            # Use config manager to get proper paths
...
intelligence_hub = TrinityIntelligenceHub()